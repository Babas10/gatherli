...
void main() {
  ...
  group('FeedbackDisplayWidget Widget Tests', () {
    testWidgets(
      'shows loading indicator when state is LoadingAggregatedFeedback',
      (tester) async {
        when(
          () => mockBloc.state,
        ).thenReturn(const LoadingAggregatedFeedback('session-123'));
    ...
    testWidgets('retry button adds LoadAggregatedFeedback event', (
      ...
      verify(
        () => mockBloc.add(const LoadAggregatedFeedback('session-123')),
      ).called(2); // Once in initState, once from button
    });

    testWidgets(
      'shows empty state when no feedback and user has not submitted',
      (tester) async {
    ...
    testWidgets('sends LoadAggregatedFeedback event in initState', (
      ...
      when(
        () => mockBloc.stream,
      ).thenAnswer((_) => Stream.value(const FeedbackInitial()));
      when(() => mockBloc.add(any())).thenReturn(null);
