...
void main() {
  group('RegistrationBloc', () {
    ...
    group('RegistrationSubmitted', () {
      ...
      group('Successful Registration', () {
        blocTest<RegistrationBloc, RegistrationState>(
          ...
          verify: (_) {
            verify(
              () => mockAnalytics.logEvent(name: 'onboarding_completed'),
            ).called(1);
          },
        );
          ...
          setUp: () {
            ...
            mockAuthRepository.setUpdateUserNamesBehavior(
              ({
                ...
                String? gender,
              }) async => throw Exception('Failed to persist names'),
            );
          },
          build: () => registrationBloc,
          act: (bloc) => bloc.add(
    ...
    group('State equality', () {
      ...
      test(
        'RegistrationFailure instances with different messages are not equal',
        () {
          expect(
    ...
    group('Event equality', () {
      ...
      test(
        'RegistrationSubmitted instances with different values are not equal',
        () {
          expect(
            const RegistrationSubmitted(
              firstName: 'John',
