...
void main() {
  ...
  group('Profile Management - Display Name Update', () {
    ...
    test('Empty display name update is rejected', () async {
      // 1. Create a test user
      final user = await FirebaseEmulatorHelper.createCompleteTestUser(
  ...
  group('Profile Management - Photo URL Update (Avatar)', () {
    test('User can update their photo URL', () async {
      // 1. Create a test user
      final user = await FirebaseEmulatorHelper.createCompleteTestUser(
      ...
      await FirebaseFirestore.instance.collection('users').doc(user.uid).update(
        {'photoUrl': newPhotoUrl, 'updatedAt': FieldValue.serverTimestamp()},
      );
    ...
    test('User can remove their photo URL', () async {
      ...
      expect(
        ...
        equals('https://example.com/old-avatar.jpg'),
      );
  ...
  group('Profile Management - Locale/Timezone Preferences', () {
    test('User can save locale preferences to Firestore', () async {
      // 1. Create a test user
      final user = await FirebaseEmulatorHelper.createCompleteTestUser(
    ...
    test('User can update timezone preference', () async {
    ...
    test('User can change locale language', () async {
      // 1. Create a test user
      final user = await FirebaseEmulatorHelper.createCompleteTestUser(
  ...
  group('Profile Management - Privacy Settings', () {
    test('User can update privacy level to friends-only', () async {
      ...
      await FirebaseFirestore.instance.collection('users').doc(user.uid).update(
        {'privacyLevel': 'friends', 'updatedAt': FieldValue.serverTimestamp()},
      );
    ...
    test('User can update privacy level to private', () async {
      // 1. Create a test user with public privacy
      final user = await FirebaseEmulatorHelper.createTestUser(
    ...
    test('User can hide email while keeping phone visible', () async {
      ...
      // 2. Hide email only
      await FirebaseFirestore.instance.collection('users').doc(user.uid).update(
        {'showEmail': false, 'updatedAt': FieldValue.serverTimestamp()},
      );
  ...
  group('Profile Management - Notification Preferences', () {
    test('User can disable all notifications', () async {
    ...
    test('User can selectively enable push notifications only', () async {
  ...
  group('Profile Management - Extended Profile Fields', () {
    test('User can update bio and location', () async {
      // 1. Create a test user
      final user = await FirebaseEmulatorHelper.createCompleteTestUser(
    ...
    test('User can update first name and last name', () async {
      // 1. Create a test user
      final user = await FirebaseEmulatorHelper.createCompleteTestUser(
    ...
    test('User can update phone number', () async {
      // 1. Create a test user
      final user = await FirebaseEmulatorHelper.createCompleteTestUser(
  ...
  group('Profile Management - Error Handling', () {
    test('Updating non-existent user document throws error', () async {
    ...
    test('Cannot read profile when not authenticated', () async {
      // 1. Create a test user
      final user = await FirebaseEmulatorHelper.createCompleteTestUser(
    ...
    test('Invalid field value types are handled', () async {
      // 1. Create a test user
      final user = await FirebaseEmulatorHelper.createCompleteTestUser(
    ...
    test('Concurrent profile updates preserve latest value', () async {
      // 1. Create a test user
      final user = await FirebaseEmulatorHelper.createCompleteTestUser(
  ...
  group('Profile Management - UserModel Serialization', () {
    test('UserModel can be serialized to and from Firestore', () async {
