...
void main() {
  ...
  group('Training Session Creation', () {
    ...
    test('User can create a training session within a group', () async {
      ...
      final sessionRef = await firestore.collection('trainingSessions').add({
        ...
        'location': {'name': 'Beach Court 1', 'address': '123 Beach Blvd'},
        'minParticipants': 4,
        'maxParticipants': 12,
    ...
    test('Training session stores location correctly', () async {
      await createTestGroupAndUser();
    ...
    test('Training session stores participant limits correctly', () async {
      await createTestGroupAndUser();
    ...
    test('Training session time is stored correctly', () async {
      await createTestGroupAndUser();
      ...
      final storedStartTime = (sessionDoc.data()?['startTime'] as Timestamp)
          .toDate();
      final storedEndTime = (sessionDoc.data()?['endTime'] as Timestamp)
          .toDate();
    ...
    test('Multiple training sessions can be created in same group', () async {
      await createTestGroupAndUser();
  ...
  group('Training Session Participation', () {
    ...
    test('User can join a training session', () async {
    ...
    test('User can leave a training session', () async {
      await createTestUsersAndGroup();
      ...
      await firestore
          ...
          .collection('participants')
          .doc(participantId)
          .update({'status': 'left', 'leftAt': FieldValue.serverTimestamp()});
    ...
    test('Creator remains in participant list after creation', () async {
      await createTestUsersAndGroup();
  ...
  group('Exercise Management', () {
    ...
    test('User can add exercise to training session', () async {
  ...
  group('Feedback Submission', () {
    ...
    test('User can submit feedback for a training session', () async {
      await createTestSessionWithGroup();
    ...
    test('Multiple feedback entries can be submitted for same session', () async {
      await createTestSessionWithGroup();
    ...
    test('Feedback ratings are within valid range', () async {
      await createTestSessionWithGroup();
  ...
  group('Training Session Status', () {
    ...
    test('Training session status can be updated to cancelled', () async {
      await createTestGroupAndUser();
    ...
    test('Training session status can be updated to completed', () async {
      await createTestGroupAndUser();
