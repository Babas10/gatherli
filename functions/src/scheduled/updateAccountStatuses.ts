...
export const updateAccountStatuses = functions.region('europe-west6').pubsub
  ...
  .onRun(async () => {
    ...
    try {
      ...
      // Auth lookups are independent per-user — run them in parallel instead
      // of awaiting one at a time. batch.update() calls are synchronous and
      // safe to issue from within the parallel map.
      await Promise.all(
